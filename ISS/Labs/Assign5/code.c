...
void rsa_init()
{
    ...
    while (1)
    {
        p = random() & BIT_MASK_16;
        q = random() & BIT_MASK_16;
...
int main()
{
    ...
    printf("Binary data %" PRIu32 "\n\n", data);

    uint32_t enc = rsa_encrypt(data);
    printf("Signature %" PRIu32 "\n", enc);
    printf("Message %s\n", data_str);

    // VERIFIER HAS E and N of PUBLIC KEY
    uint32_t enc_ver = rsa_encrypt(data);
    printf("Verifier Signature %" PRIu32 "\n", enc_ver);
